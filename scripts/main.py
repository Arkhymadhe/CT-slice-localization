...
def configure_args():
    ...
    args.add_argument('--arch_dir', type = str, help = 'Directory for generated compressed data file',
                        default = os.path.join(os.getcwd().replace('scripts', 'data'), 'ct-dataset.zip'))
    
    args.add_argument('--style', type = str, default = 'gruvboxd', help = 'Visualization style',
                        choices = ['gruvboxd', 'solarizedd', 'solarizedl', 'normal', 'chesterish'])
    
    args.add_argument('--epochs', type = int, default = 20, help = 'Number of complete data rounds')
    ...
    args.add_argument('--cardinality', type = int, default = 50, help = 'Max number of possible classes')
    
    args.add_argument('--var_lim', type = float, default = 0.8, help = 'Max allowable limit on variance')
    ...
    args.add_argument('--valid', type = bool, choices = [True, False], default = True,
                        help = 'Validation split?')
    
    args.add_argument('--save', type = bool, default = True, choices = [True, False],
                        help = 'Save model artefacts?')
    
    args.add_argument('--artefact_dir', type = str, default = os.getcwd().replace('scripts', 'artefacts'),
                        help = 'Location for saved model')
    
    args.add_argument('--task', default = 'regression', type = str, choices = ['classif', 'regression'],
...
def main():
    ...
    drop_labels = get_invariant_features(data.iloc[:, :-1], cardinality = args.cardinality,
                                         percent = args.var_lim)
    ...
    if args.valid:
        X_train, X_val, y_train, y_val = split_data(X_train, y_train, split_size = args.t2)
    ...
    model = import_model(task = args.task, n_features = args.n_features, n_classes = args.classes,
                         ...
                         batch_size = args.batch_size, split = None)
