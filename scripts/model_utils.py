...
def import_model(optim, task : str, n_features : int,
    ...
    if task == 'classif':
        final_net = NeuralNetClassifier(module = net, optimizer = optim, optimizer__lr = lr,
                                        max_epochs = max_epochs, batch_size = batch_size, train_split = split)
    else:
        final_net = NeuralNetRegressor(module = net, optimizer = optim, optimizer__lr = lr,
                                       max_epochs = max_epochs, batch_size = batch_size, train_split = split)
    return final_net
