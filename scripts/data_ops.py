...
def read_compress(path_to_data, path_to_archive="compressed_data.zip"):
    ...
    with zipfile.ZipFile(path_to_archive, "w") as zf:
        zf.write(
            path_to_data, arcname="ct_dataset.csv", compress_type=zipfile.ZIP_DEFLATED
        )
        zf.close()
...
def variables(data, labels, target: str, axis=1, return_y=True):
    ...
    assert type(target) in [
        str,
        list,
    ], "Target must be a feature name (str) or list of feature names."
...
def array_to_tensor(array):
    ...
    assert type(array) in [
        pd.DataFrame,
        pd.Series,
        np.ndarray,
    ], "Array must be of type `pd.DataFrame`, `pd.Series`, or `np.ndarray`."
...
def split_data(X, y, split_size=0.2, stratify=False):
    ...
    X_1, X_2, y_1, y_2 = train_test_split(
        X, y, test_size=split_size, stratify=y if stratify else None
    )
